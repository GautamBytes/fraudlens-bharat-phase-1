...
It reports per-class metrics, confusion matrices, expected calibration error
where applicable, coverage, and an explicit non-timed latency field so the JSON
remains deterministic. Rule confusion matrices carry an `unknown` column so
abstentions are visible and count as overall errors. The marker model is a
